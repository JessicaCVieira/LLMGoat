...
'''
Using Ollama (mistral only — no embedding model required)
'''
...
REQUEST_TIMEOUT = 120  # seconds — prevent indefinite hang
...
def generate_response_stream(prompt):
    ...
    context = _load_context("llms/llm1/docs.txt")
    ...
    print(f"[LLM1] Sending request to Ollama ({OLLAMA_CHAT_MODEL})...")
    try:
        with requests.post(
            ...
            r.raise_for_status()
            print("[LLM1] Connected. Streaming response...")
            for line in r.iter_lines():
                if line:
                    try:
                        chunk = json.loads(line.decode("utf-8"))
